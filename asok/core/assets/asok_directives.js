    ...
    if (root === document) {
      ...
      if (islands.length > 0) {
        ...
        const initIsland = (island) => {
          if (island._stateInitialized || island.__asokIslandInited) return;
          island.__asokIslandInited = true;
          if (island.hasAttribute('asok-state-ref')) {
            initState(island);
          }
          init(island);
        };
