...
def test_table_attribute_merging_reactive():
    ...
    table.columns = [
        TableColumn(
            ...
            label="User Name",
            sortable=True,
            th__class="specific-th",
            td__class="specific-td",
        ),
        TableColumn("status"),
    ]
    ...
    assert (
        'class="custom-container"' in html_out
        or 'class="asok-table-container custom-container"' in html_out
    )
    assert 'id="main-container"' in html_out

    # Ensure class="custom-container" is not duplicated
    assert (
        html_out.count('class="custom-container"') == 1
        or html_out.count('class="asok-table-container custom-container"') == 1
    )

    # Ensure table element has merged classes and the custom ID
    assert 'id="the-table-el"' in html_out
    assert (
        'class="asok-table custom-table"' in html_out
        or 'class="custom-table asok-table"' in html_out
    )

    # Ensure sortable column has merged th classes
    assert (
        'class="asok-sortable global-th specific-th"' in html_out
        or "global-th specific-th" in html_out
    )
    ...
    duplicate_classes = re.findall(
        r'<[a-z0-9]+(?:\s+[^>]*?\s*class="[^"]*"){2,}', html_out
    )
    assert len(duplicate_classes) == 0, (
        f"Found tags with duplicate class attributes: {duplicate_classes}"
    )
...
def test_table_attribute_merging_non_reactive():
    ...
    assert (
        'class="custom-container-nr"' in html_out
        or 'class="asok-table-container custom-container-nr"' in html_out
    )
    assert 'class="asok-table custom-table-nr"' in html_out

    # Check that specific th classes were merged with global th classes
    assert (
        "global-th-nr specific-th-nr" in html_out
        or "specific-th-nr global-th-nr" in html_out
    )

    # Check that specific td classes were merged with global td classes
    assert (
        "global-td-nr specific-td-nr" in html_out
        or "specific-td-nr global-td-nr" in html_out
    )
...
def test_statement_handling_in_registry():
    ...
    assert is_stmt is True, (
        "Expression containing 'if(...)' should be detected as a statement"
    )
...
def test_master_checkbox_binding_reactive():
    ...
    html_out = table.render()
    assert (
        'asok-bind:checked="selected.length === items.length && items.length > 0"'
        in html_out
    )
